; (function (fns) {
    ...
    function ValuePromise(value) {
        this.then = function (onFulfilled) {
            ...
            return new fns.Promise(function (resolve, reject) {
                asap(function () {
                    try {
    ...
    ValuePromise.prototype = fns.Promise.prototype;
    ...
    fns.Promise.resolve = function (value) {
        if (value instanceof fns.Promise) return value;
        ...
        if (typeof value === "object" || typeof value === "function") {
            try {
                var then = value.then;
                if (typeof then === "function") {
                    return new fns.Promise(then.bind(value));
                }
            } catch (ex) {
                return new fns.Promise(function (resolve, reject) {
                    reject(ex);
    ...
    fns.Promise.reject = function (value) {
        return new fns.Promise(function (resolve, reject) {
            reject(value);
        });
    };
    fns.Promise.race = function (values) {
        return new fns.Promise(function (resolve, reject) {
            for (var key in values) {
                // 将数组元素转换为promise对象
                // 由resolve函数内部的标识位done来保证onFulfilled函数仅执行过一次
                fns.Promise.resolve(values[key]).then(resolve, reject)
            }
        });
    };
    ...
    fns.Promise.prototype["catch"] = function (onRejected) {
        return this.then(null, onRejected);
    };
    //Promise是ES6中的内容，如果没有则浏览器的版本较老
