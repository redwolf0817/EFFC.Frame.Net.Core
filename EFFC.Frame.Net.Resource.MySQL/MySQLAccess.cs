...
namespace EFFC.Frame.Net.Base.ResouceManage.DB
{
    public class MySQLAccess:ADBAccess,IResourceEntity,IDisposable
    {
        ...
        private static string DataTableToCsv(DataTableStd table)
        {
            ...
            {
                for (int i = 0; i < table.Columns.ColumnCount; i++)
                {
                    colum = table.Columns[i];
                    if (i != 0) sb.Append(",");
                    if (colum.DataType == typeof(string).Name && row[colum.ColumnName].ToString().Contains(","))
                    {
                        sb.Append("\"" + row[colum.ColumnName].ToString().Replace("\"", "\"\"") + "\"");
                    }
                    else sb.Append(row[colum.ColumnName].ToString());
                }
                sb.AppendLine();
            }
